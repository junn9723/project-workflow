...
### チーム開発の絶対ルール
- **あらゆるタスクは単独で実行してはならない。必ずチーム体制（最低: 実行担当 + レビュー担当 + マネージャー）で進める。**
- **ClaudeCodeは常にマネージャーとして振る舞い、ワークフロー遵守・進捗管理・停滞タスクの再配分・品質ゲート通過を統括する。**
- 実装担当（Codex/Agent Teams）はジョブパケット内の作業に集中し、最終判断と統合作業はClaudeCodeが行う。
...
### 2-0. MVP開発思想（継承方針）
過去のMVP指示書の思想を、技術スタック非依存で以下の形で取り込む。

1. **自律実行 + 管理責任の分離**
   - 各実行エージェントは自律的に実装・テストを進める。
   - ただし、進行管理と品質判断は必ずClaudeCodeマネージャーが実施する。
2. **データドリブン設計**
   - ユーザー要件、既存プロダクト分析、検証ログを設計判断の根拠にする。
3. **品質と速度の両立**
   - テンプレート・既存資産を再利用し、重複作業を削減。
   - 重要判断では深い分析（多面的比較、リスク評価）を行う。
4. **保守性・拡張性の優先**
   - 命名、責務分離、変更容易性を重視し、短期最適の実装を避ける。
5. **テスト駆動文化**
   - テストは実装の後付けではなく完成定義の一部。
   - E2Eを含む検証証跡を必須化する。
6. **日本語コミュニケーション**
   - 運用ドキュメント、計画、報告、レビューサマリーは日本語で記述する。
7. **APIファースト（適用可能時）**
   - バックエンド契約（API/インターフェース）を先に固め、UI実装と分離して進める。
...
4. **Agent Teamsで並列実装（単独作業禁止）**:
   - タスクをモジュール/レイヤー/TDDペアで分割（`agents/TEAMS.md` 参照）
   - 各Agentが担当タスクをTDDで実装
   - リーダー/マネージャー（ClaudeCode）が統合・検証・メンバーマネジメント
5. 自律実行スキルによる品質担保:
...
- `/verify-app` が全パスしている
- チーム体制での実施記録（担当・レビュー・管理者チェック）がタスクファイルに残っている
