...
## 1-0. チーム実行ポリシー（絶対）
- **あらゆるタスクは単独で行わない。** 最低でも「実行担当」「レビュー担当」「ClaudeCode（マネージャー）」の3ロールで実施する。
- 実行担当は実装・検証を行い、レビュー担当は受け入れ条件と変更差分を確認する。
- ClaudeCodeは最終的な進行判断・統合判断を行い、逸脱タスクを是正する。

### 1-1. オーケストレータ（ClaudeCode）
- **マネージャー責務（必須）**: ワークフロー遵守の監督、メンバーの進捗管理、停滞タスクの再配分、品質ゲート通過判断を担う
